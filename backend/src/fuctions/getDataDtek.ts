...
export const getDataDtek = async (company: string) => {
  ...
  const textSelector = await page.waitForSelector(
    '.indicators-ent__form > :nth-child(4) > .basic-table > .basic-table__body > tr > td > div > :nth-child(2)'
  );
  ...
  if (company === 'Orel') {
    ...
    await page.waitForSelector(
      '.sidebar-Kem__nav > :nth-child(3) > .sidebar-Kem__nav-sub > :nth-child(1)'
    );
    await page.click(
      '.sidebar-Kem__nav > :nth-child(3) > .sidebar-Kem__nav-sub > :nth-child(1)'
    );
    ...
    const invoiceAmountSelector = await page.waitForSelector(
      '.basic-table__body > :nth-child(4)  > :nth-child(4) '
    );
    const invoiceAmount = await invoiceAmountSelector?.evaluate(
      (el) => el.textContent
    );
    resMegan.invoiceAmount = invoiceAmount;

    await page.waitForSelector(
      '.month-info-m > :nth-child(4)  > :nth-child(3)'
    );
    await page.click('.month-info-m > :nth-child(4)  > :nth-child(3)');

    // const aSelector = await page.waitForSelector(
    //   '.month-info-m > :nth-child(4)  > :nth-child(3) '
    // );

    // const a = await aSelector?.evaluate((el) => el.textContent);
    // console.log('A', a);
    // await sleep(10000);
